...
#         scheduler = [torch.optim.lr_scheduler.StepLR(optimizer[0], step_size=1, gamma=0.3)]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for j in range(sections):
            ...
            output = output = self(torch.cat([batch.cell_data, batch.x], axis=-1), batch.e_radius[:, subset_ind], emb).squeeze() if ('ci' in self.hparams["regime"]) else self(batch.x, batch.e_radius[:, subset_ind], emb).squeeze()
...
class FilterBaseBalanced(LightningModule):
