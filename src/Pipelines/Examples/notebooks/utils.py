...
def evaluate_set_metrics(r, dataset, model, chkpnt, test_size):
    ...
    for i, batch in enumerate(test_dataset[:test_size]):
        ...
        loss = torch.nn.functional.hinge_embedding_loss(d, hinge, margin=1, reduction="mean")
...
def model_evaluation(model, test_dataset, test_size=10, fom= "eff", fixed_value=0.96):
    ...
    sol = root(evaluate_set_root, args=(test_dataset, model, chkpnt, 1, fixed_value, fom), x0=0.5, x1=1.)
    print("Seed solver complete, radius:", sol.root)
    sol = root(evaluate_set_root, args=(test_dataset, model, chkpnt, test_size, fixed_value, fom), x0=sol.root, x1=sol.root*0.9)
    print("Final solver complete, radius:", sol.root)
    
    # Return ( (efficiency, purity), radius_size)
    return evaluate_set_metrics(sol.root, test_dataset, model, chkpnt, test_size), sol.root