...
class EdgeNetwork(nn.Module):
    ...
    unpool_description = namedtuple("UnpoolDescription", ["cluster", "node_weights"])
    ...
    def merge_edges(
        ...
        while i < contract_iters and ratio > contract_stop_cut:
            ...
            max_score_0, max_indices_0 = scatter_max(
                edge_score, edge_index[0], dim=0, dim_size=x.shape[0]
            )
            max_score_1, max_indices_1 = scatter_max(
                edge_score, edge_index[1], dim=0, dim_size=x.shape[0]
            )

            # stack scores for each direction
            stacked_score, stacked_indices = (
                torch.stack([max_score_0, max_score_1]),
                torch.stack([max_indices_0, max_indices_1]).T,
            )
            ...
            max_indices[max_score_0 > max_score_1] = edge_index[1][
                max_indices_0[max_score_0 > max_score_1]
            ]
            max_indices[max_score_1 > max_score_0] = edge_index[0][
                max_indices_1[max_score_1 > max_score_0]
            ]
            ...
            edge_index_match = (
                edge_index_match_0 & edge_index_match_1 & node_0_valid & node_1_valid
            )
            ...
            edge_score_zero_mask = (
                (edge_index[..., None] == nodes[nodes_removed]).any(-1).any(0)
            )
        ...
        remaining_indices = edges_contracted.shape[1] + torch.arange(
            torch.sum(nodes_remaining)
        ).to(device)
        new_node_index_map = torch.cat(
            [
                ...
                torch.stack([nodes[nodes_remaining], remaining_indices]),
            ],
            dim=-1,
        )
        ...
        if duplicates[0].size(0) > 0:
            ...
            for i in d:
                ...
                cluster_remove_mask = ~(
                    duplicate_mask & (cluster != valid_cluster_index)
                )
        ...
        unpool_info = self.unpool_description(
            cluster=cluster, node_weights=new_node_weights
        )
    ...
    def forward(self, x, edge_index):
        ...
        for i in range(self.hparams["outer_contract_iters"]):
            ...
            (
                ...
                new_edge_scores,
                unpool_info,
            ) = self.edge_network.merge_edges(
                x,
                edge_index,
                batch,
                new_edge_scores,
                edge_scores,
                self.hparams["contract_edge_cut"],
                self.hparams["contract_stop_cut"],
                self.hparams["inner_contract_iters"],
            )
            ...
            input_x = scatter_add(
                ...
                unpool_info.cluster,
                dim=0,
                dim_size=torch.unique(unpool_info.cluster).size(0),
            )
