...
class InteractionMultistepGNN(GNNBase):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if ('pid' in self.hparams["regime"]):
            ...
            loss = F.binary_cross_entropy_with_logits(torch.cat(output), y_pid.float(), pos_weight = weight)
        else:
            y = batch.y.repeat((self.hparams["n_graph_iters"]))
            loss = F.binary_cross_entropy_with_logits(torch.cat(output), y, pos_weight = weight)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output = (self(torch.cat([batch.cell_data, batch.x], axis=-1), batch.edge_index)
                  if ('ci' in self.hparams["regime"])
                  else self(batch.x, batch.edge_index))

        if ('pid' in self.hparams["regime"]):
            y_pid = (batch.pid[batch.edge_index[0]] == batch.pid[batch.edge_index[1]]).float()
            val_loss = F.binary_cross_entropy_with_logits(torch.cat(output), y_pid.float().repeat((self.hparams["n_graph_iters"])), pos_weight = weight)
        else:
            y = batch.y
            val_loss = F.binary_cross_entropy_with_logits(torch.cat(output), y.repeat((self.hparams["n_graph_iters"])), pos_weight = weight)
...
class CheckpointedInteractionMultistepGNN(InteractionMultistepGNN):
    ...
    def forward(self, x, edge_index):
        ...
        for i in range(self.hparams["n_graph_iters"]):
            ...
            classifier_inputs = torch.cat([x[start], x[end], e], dim=1)
            edge_outputs.append(checkpoint(self.output_edge_classifier, classifier_inputs).squeeze(-1))
