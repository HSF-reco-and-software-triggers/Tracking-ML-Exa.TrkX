...
class GravConv(nn.Module):
    def __init__(self, hparams, input_size=None, output_size=None):
        ...
        # This handles some extra ideas that might be interesting to try
        self.setup_extended_architectures(hparams)
    ...
    def get_neighbors(self, spatial_features):
        ...
        if self.use_radius:
            radius_edges = build_edges(spatial_features, spatial_features, r_max=self.r, k_max=self.hparams["max_knn"])
    ...
    def grav_weight(self):
        ...
        if "grav_weight" not in self.hparams:
            ...
            return ( self.hparams["grav_weight"][0] + ( (self.hparams["grav_weight"][1] - self.hparams["grav_weight"][0]) * self.current_epoch / self.hparams["max_epochs"] ) ) * grav_weight_multiplier
        elif len(self.hparams["grav_weight"]) == 3:
            return (self.hparams["grav_weight"][0] + ((self.hparams["grav_weight"][1] - self.hparams["grav_weight"][0]) * self.current_epoch / (self.hparams["max_epochs"] / 2))) * grav_weight_multiplier if self.current_epoch < self.hparams["max_epochs"] / 2 else (self.hparams["grav_weight"][1] + ((self.hparams["grav_weight"][2] - self.hparams["grav_weight"][1]) * (self.current_epoch - self.hparams["max_epochs"] / 2) / (self.hparams["max_epochs"] / 2))) * grav_weight_multiplier
