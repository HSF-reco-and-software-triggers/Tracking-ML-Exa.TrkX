...
def load_dataset(input_subdir="", num_events=10, pt_background_cut=0, pt_signal_cut=0, noise=False, triplets=False, **kwargs):
...
def process_data(events):
    ...
    for i, event in enumerate(events):   
        ...
        event.label_pairs, event.pid_pairs = get_segment_pairs(event, long_labels, long_pid)    
        
    return events

def get_long_segments(event):
    
    long_segments, segment_inverse, segment_counts = event.labels.unique(return_counts=True, return_inverse=True)
...
def get_segment_pairs(event, long_labels, long_pid):
    ...
    matching = pd.DataFrame(np.array([np.concatenate([[124], long_labels.numpy()]), np.concatenate([[0], long_pid.numpy()]), np.ones(len(long_labels)+1)]).T, columns=["labels", "pid", "count"])
    max_pid_df = matching.groupby(["labels","pid"], as_index=False).count().sort_values("count").drop_duplicates(["labels"], keep="last")
