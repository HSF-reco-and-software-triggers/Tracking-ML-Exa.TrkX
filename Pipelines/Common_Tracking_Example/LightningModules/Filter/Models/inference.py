...
class FilterBuilder(Callback):        
    ...
    def construct_downstream(self, batch, pl_module, datatype):
        ...
        for j in range(pl_module.hparams["n_chunks"]):
            ...
            output = (
                pl_module(
                    torch.cat([batch.cell_data[:, :pl_module.hparams["cell_channels"]], batch.x], axis=-1),
        ...
        if ("weights" in batch.__dict__.keys()) and (batch.weights.shape == cut_list.shape):
            batch.weights = batch.weights[cut_list]
        
        print("eff", torch.tensor(edge_true_positive / edge_true),
