...
class DirectedGravMetric(SandboxEmbeddingBase):
    ...
    def forward(self, x):
        ...
        topo_edges = build_edges(topo_a, topo_b, r_max=self.hparams["latent_margin"], k_max=self.hparams["latent_k"])
    ...
    def get_potential(self, x_a, x_b, edges):
        ...
        potential = (torch.exp(1 - d_sq / self.hparams["latent_margin"]**2) - 1) / (np.exp(1) - 1)

        return potential        

class UndirectedGravMetric(SandboxEmbeddingBase):
    def __init__(self, hparams):
        ...
        if self.hparams["node_update"] == "feat":
            feature_size = hparams["feature_hidden"] + hparams["spatial_channels"] + hparams["cell_channels"]
        elif self.hparams["node_update"] == "topo":
            feature_size = hparams["emb_dim"] + hparams["spatial_channels"] + hparams["cell_channels"]
        elif self.hparams["node_update"] == "topofeat":
            feature_size = hparams["emb_dim"] + hparams["feature_hidden"] + hparams["spatial_channels"] + hparams["cell_channels"]
    ...
    def forward(self, x):
        ...
        topo_edges = build_edges(topo, topo, r_max=self.hparams["latent_margin"], k_max=self.hparams["latent_k"])

        # 2. Get gravity potentials and weight features as attention mechanism
        edge_potentials = self.get_potential(topo, topo, topo_edges)
        if self.hparams["feature_hidden"] > 0 and (self.hparams["node_update"] in ["feat", "topofeat"]):
            features_in = self.feature_mlp(x)
            if self.hparams["node_update"] == "feat":
                weighted_features_in = features_in[topo_edges[0]] * edge_potentials.unsqueeze(-1)
            elif self.hparams["node_update"] == "topofeat":
                weighted_features_in = torch.cat([features_in, topo], dim=-1)[topo_edges[0]] * edge_potentials.unsqueeze(-1)
        else:
            weighted_features_in = topo[topo_edges[0]] * edge_potentials.unsqueeze(-1)
            
        if self.hparams["aggregation"] == "mean":
            neighborhood_in = scatter_mean(weighted_features_in, topo_edges[1], dim=0, dim_size=topo.shape[0]) 
        else:
            neighborhood_in = scatter_add(weighted_features_in, topo_edges[1], dim=0, dim_size=topo.shape[0])  
            
            if self.hparams["aggregation"] == "weighted_sum":
                mean_weights_in = scatter_add(edge_potentials, topo_edges[1], dim=0, dim_size=topo.shape[0])
                neighborhood_in[mean_weights_in != 0] = neighborhood_in[mean_weights_in != 0] / mean_weights_in[mean_weights_in != 0].unsqueeze(-1)
                
        # 4. Pass [mean_neighbors_a, topo_a, topo_b, mean_neighbors_b] to map_1a, map_1b
        out = F.normalize(self.map_1(torch.cat([neighborhood_in, x], dim=-1)))
        # out = F.normalize(self.map_1(torch.cat([neighborhood_in], dim=-1)))
...
class SemiDirectedGravMetric(SandboxEmbeddingBase):
    def __init__(self, hparams):
        print("SemiDirectedGravMetric")
        super().__init__(hparams)
        """
        An implementation of the GravMetric architecture: The most naive version,
        such that input maps are undirected, and there is only topo-space.
    ...
    def forward(self, x):
        ...
        # topo_edges = build_edges(topo_a, topo_b, r_max=self.hparams["latent_margin"], k_max=self.hparams["latent_k"])
        if self.hparams["use_knn"]:
            topo_edges = build_knn(topo_a, topo_b, k_max=self.hparams["latent_k"])
        ...
        # 4. Pass [mean_neighbors_a, topo_a, topo_b, mean_neighbors_b] to map_1a, map_1b
        out = F.normalize(self.map_1(torch.cat([neighborhood_in, x, neighborhood_out], dim=-1)))

        return [(topo_a, topo_b), out]
