...
class MultiInteractionGNN(GNNBase):
    ...
    def __init__(self, hparams):
        ...
        # The edge network computes new edge features from connected nodes
        self.edge_networks = nn.ModuleList([make_mlp(
            3 * hparams["hidden"],
            [hparams["hidden"]] * hparams["nb_edge_layer"],
            layer_norm=hparams["layernorm"],
            output_activation=None,
        ...
        self.node_networks = nn.ModuleList([make_mlp(
    ...
    def forward(self, x, edge_index):
        ...
        for i in range(self.hparams["n_graph_iters"]):
            ...
            x, e = checkpoint(self.message_step, self.node_networks[i], self.edge_networks[i], x, start, end, e)
