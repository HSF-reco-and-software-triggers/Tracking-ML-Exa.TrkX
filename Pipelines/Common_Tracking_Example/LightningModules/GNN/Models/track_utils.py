...
def get_metrics_matrices(edge_scores, particles_df, truth_df, edge_cut_range = None, track_length_range = None, ROI = None):
...
def obtain_target_fake_rate(efficiency_matrix, fake_rate_matrix, edge_cut_range, track_length_range, target_fake_rate = 1e-3):
    ...
    assert (fake_rate_matrix < target_fake_rate).sum() > 0, "Cannot find any tracking values passing target fake rate"
    
    fake_rates_passing_cut = fake_rate_matrix < target_fake_rate
    best_fake_rate_arg = np.argmin(np.abs(fake_rate_matrix[fake_rates_passing_cut] - target_fake_rate))
