...
def plot_track(ts, ps, actual, zs, cov, std_scale=1,
    ...
    bp.plot_track(ts,actual, c='k')
#    bp.plot_measurements(range(1, count + 1), zs)
#    bp.plot_filter(range(1, count + 1), ps)    
    bp.plot_measurements(ts, zs)
    bp.plot_filter(ts, ps)

    plt.plot(ts, std_top, linestyle=':', color='k', lw=1, alpha=0.4)
    plt.plot(ts, std_btm, linestyle=':', color='k', lw=1, alpha=0.4)
    plt.fill_between(ts, std_top, std_btm,
