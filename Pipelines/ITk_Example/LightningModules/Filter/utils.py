...
class filter_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.hparams["ratio"] != 0:
            num_true, num_false = batch.modulewise_true_edges.shape[1], (~batch.y.bool()).sum()
            ...
            combined_edges = torch.cat([batch.modulewise_true_edges, batch.modulewise_true_edges.flip(0), random_edges], dim=1)
            combined_y = torch.cat([torch.ones(2*batch.modulewise_true_edges.shape[1]), batch.y[start_index:end_index]])
