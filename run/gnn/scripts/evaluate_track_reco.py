...
from run.utils.plotting_utils import plot_pt_eff
...
def evaluate(config_file="pipeline_config.yaml"):
    ...
    particles.loc[:, "is_reconstructed"] = particles.loc[:, "particle_id"].isin(grouped_reco_particles[grouped_reco_particles].index.values)
    particles = particles.drop_duplicates(subset=['particle_id'])

    # Plot the results across pT and eta
    plot_pt_eff(particles, savefig=os.path.join(evaluation_configs['output_dir'], 'pt_eff.png'))
